...
该模式只处理音频材料的选择题，不会提交答案；答案优先从 `answer_cache.json` 读取，未命中缓存时会在文档中标记为“未缓存”。导出文档会按音频/页面分组：顶部显示面包屑路径和可选音频链接，随后是听力文本，再按 `1. ( A ) 题干` 的格式列出题目和选项；不同音频之间用 Markdown 分割线隔开。
